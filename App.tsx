...
// ... (rest of normalizeProduct function remains the same) ...
...
// ... (Rest of normalized functions: normalizeInsuranceDrug, normalizeCosmetic) ...
...
// ... (Auth and Effects unchanged) ...
  ...
  const handleFindAlternative = (medicine: Medicine) => {
    ...
    // 1. By Concentration (Strength) - Ascending
    // 2. By Trade Name - Alphabetical
    
    const parseStrength = (s: string) => {
       const match = s.match(/(\d+(\.\d+)?)/);
       return match ? parseFloat(match[0]) : 0;
    };

    const sortAlternatives = (a: Medicine, b: Medicine) => {
       const strA = parseStrength(a.Strength);
       const strB = parseStrength(b.Strength);

       if (strA !== strB) {
          return strA - strB; // Ascending Strength
       }
       // Fallback to Alphabetical
       return a['Trade Name'].localeCompare(b['Trade Name']);
    };

    const direct = medicines
        .filter(m => m['Scientific Name'] === medicine['Scientific Name'] && m.RegisterNumber !== medicine.RegisterNumber)
        .sort(sortAlternatives);

    const therapeutic = medicines
        .filter(m => m.AtcCode1 && medicine.AtcCode1 && m.AtcCode1 === medicine.AtcCode1 && m['Scientific Name'] !== medicine['Scientific Name'])
        .sort(sortAlternatives);
  ...
  // ... (rest of App.tsx component remains unchanged from previous version)
