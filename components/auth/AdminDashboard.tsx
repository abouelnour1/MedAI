...
import { TFunction, User, Medicine, AppSettings } from '../../types';
...
type Panel = 'overview' | 'users' | 'medicines' | 'settings';

const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode }> = ({ title, value, icon }) => (
    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl flex items-center gap-4">
        <div className="bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary-light p-3 rounded-full">{icon}</div>
        <div>
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ t, allMedicines, setMedicines }) => {
  ...
  const handleMedicineFormSubmit = (e: React.FormEvent) => {
    ...
    if (allMedicines.some(m => m.RegisterNumber === editingMedicine.RegisterNumber && m.RegisterNumber !== editingMedicine.RegisterNumber)) {
        alert("Register number must be unique.");
        return;
    }
  ...
  const openMedicineModal = (medicine: Medicine | null) => {
    if (medicine) {
        ...
        setEditingMedicine({ RegisterNumber: `new-${Date.now()}`, "Trade Name": "", "Scientific Name": "", "Public price": "", PharmaceuticalForm: "", Strength: "", StrengthUnit: "", "Manufacture Name": "", "Legal Status": "Prescription", "Product type": "Human" } as Medicine);
    }
      ...
      case 'overview':
        ...
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <StatCard title={t('aiRequestsToday')} value={aiRequestsToday} icon={<div className="h-6 w-6"><ChartIcon /></div>} />
                <StatCard title={t('premiumUsers')} value={premiumUsers} icon={<UsersIcon />} />
                <StatCard title={t('adminUsers')} value={adminUsers} icon={<UsersIcon />} />
            </div>
        );
      case 'users':
        return (
            <div>
                 <div className="relative mb-4">
                    ...
                    <div className="absolute top-1/2 left-3 -translate-y-1/2 text-light-text-secondary dark:text-dark-text-secondary"><SearchIcon /></div>
                </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs uppercase bg-slate-50 dark:bg-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-3">{t('aiRequests')}</th>
                        ...
                        <tbody>
                            {filteredUsers.map(user => (
                                <tr key={user.id} className="border-b dark:border-slate-700">
                                    ...
                                    <td className="px-4 py-3">{user.aiRequestCount}</td>
            ...
            <div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs uppercase bg-slate-50 dark:bg-slate-800">
                            <tr>
                                <th className="px-4 py-3">{t('tradeName')}</th>
                                <th className="px-4 py-3">{t('scientificName')}</th>
                        ...
                        <tbody>
                            {filteredMedicines.map(med => (
                                <tr key={med.RegisterNumber} className="border-b dark:border-slate-700">
                                    <td className="px-4 py-3 font-medium">{med['Trade Name']}</td>
                                    <td className="px-4 py-3">{med['Scientific Name']}</td>
                                    <td className="px-4 py-3">{med['Public price']}</td>
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex gap-2 justify-end">
                                            <button onClick={() => openMedicineModal(med)} className="font-medium text-primary hover:underline">{t('editUser')}</button>
    ...
    <div className="bg-white dark:bg-dark-card rounded-xl shadow-md space-y-4 animate-fade-in flex flex-col sm:flex-row min-h-[60vh]">
      ...
      <main className="flex-grow p-4">
       ...
       {isMedicineModalOpen && editingMedicine && (
            <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={() => setIsMedicineModalOpen(false)}>
                <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-lg w-full max-w-lg" onClick={e => e.stopPropagation()}>
                    ...
                    <form onSubmit={handleMedicineFormSubmit} className="space-y-3">
                        {/* Add all medicine fields here */}
